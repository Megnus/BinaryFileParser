...
int XML :: XMLDecode(const char* src,char* trg)
   ...
   for(int i = 0 ; i < Y ; )
      ...
      if (src[i] == '&' && src[i + 1] == '#' && tolower(src[i + 2]) == 'x')
         ...
         char d1[100] = {0};
		 //Frasse 2010-06-23. This is an attempt to handle XML with \r\n in the value strings. This code will not handle unicode characters in *nix
		 if (N < 127)
			d1[0] = (char)N;
		 /*2do 2010-12-22 removed when support for Oracle Unicode was added
		 else
		 {
#ifdef _WIN32
         wchar_t d2[100] = {0};
		 ...
		 //2009-02-24
		 //Strcat fails to concat the string if the concatination string consists of \r
		 //therefore this section was rewritten.
         //Strcat(trg + x,d1);
         //x += strlen(d1);
		 for (int z = 0; z < y; z++)
		 {
   ...
   for(;;)
      ...
      for(;;)
         ...
         if (*a1 == '/' && Nest)
			...
			//2010-04-08 Frasse. Also continune if inside comment and it's not an --> Otherwise we can't spool past commented tags
...
void XMLElement :: Export(FILE* fp,int ShowAll,int SaveMode,int TargetMode,XMLHeader* hdr,class XMLTransform* eclass,class XMLTransformData* edata)
   ...
   if (eclass == 0)
      ...
      // Write
      if (TargetMode == 0)
         fwrite(yy.operator char* (),sizeof(char),nS,fp);
      else
      if (TargetMode == 1)
...
#ifdef _WIN32
void XMLElement :: SortElements(int (_USERENTRY )(const void *, const void *))
   {
   // to all elements
...
void XMLElement :: SortVariables(int (_USERENTRY )(const void *, const void *))
   {
   // to all Variables
...
int XMLVariable :: MemoryUsage()
   ...
   // Variable size
   m += GetName(0) * sizeof(char);
   m += GetValue(0) * sizeof(char);
...
#ifdef _WIN32
#ifndef WINCE
	if (IsBadStringPtr(vn,-1))
		return false;
	if (IsBadStringPtr(vv,-1))
		return false;
#endif
#endif
	if (owner)
...
void XMLElement :: SetElementName(const char* x)
   ...
   el = new char[Sug +10];
   memset(el,0,(Sug + 10) * sizeof(char));
   XML :: XMLEncode(x,el);
   }
...
int XML :: Load(const char* file,int LoadMode,XMLTransform* eclass,class XMLTransformData* edata)
   ...
   if (LoadMode == 0) // local xml file
      ...
      if (!y)
         ...
         hdr = new XMLHeader("<?xml version=\"1.0\" encoding=\"iso-8859-1\" standalone=\"yes\" ?>");
   ...
   if (!a1)
      ...
      f = 0;
      iParseStatus = 1;
	  const char* Header = "\xEF\xBB\xBF<?xml version=\"1.0\" encoding=\"iso-8859-1\" standalone=\"yes\" ?>";
	  hdr = new XMLHeader(Header);
      a1 = d;
      a2 = a1;
      }
   else
...
void XMLElement :: Reparse(const char* elm2,int Type)
   ...
   memset(el,0,(strlen(elm) + 10) * sizeof(char));
   ...
   memset(ael,0,(Sug +10) * sizeof(char));
...
unsigned int XMLElement :: GetAllChildrenNum(unsigned int deep)
   ...
   if (deep != 0) //Frasse 2007-11-05. Added this if statement otherwise it reports the number of children on the level where it doesn't add them.
	   C += childrennum;
   return C;
   }
...
void XMLVariable :: SetName(const char* VN,bool NoDecode)
   ...
   vn = new char[Sug + 10];
   memset(vn,0,(Sug + 10) * sizeof(char));
   if (NoDecode)
      strcpy(vn,VN);
   else
      XML :: XMLEncode(VN,vn);
...
void XMLVariable :: SetValue(const char* VV,bool NoDecode)
   ...
   vv = new char[Sug + 10];
   memset(vv,0,(Sug + 10) * sizeof(char));
